...
module.exports = async function (data, socket, io) {
  ...
  try {
    const getKey = await function w() {
      return [...userinterval].find(([key, val]) => val == data.userId)[0];
    };
    const client = getKey();
    userinterval.delete(client)
  }catch{}
  const config = {
    method: "post",
    ...
    try {
      const time = await DistinationDuration(
        ...
        const category = await CategoryFareM.find({}).then(async (res) => {
          ...
          //var user_id = users.get(data.userId);
          io.to(users.get(data.userId)).emit("GetAvailableTripCategoryCar", data1);
        });
        const fun = () => {
          if (
            ...
            for (var i = 1; i <= res.length; i++) {
              ...
              const temp = await DriverM.findOne({
                ...
                if (i == 1 && driver == null) {
                  let user_id = users.get(data.userid);
                  socket.emit("GetAvailableTripCategoryCar", {
                    message: "no driver found",status:false
                  });
                } else if (driver != null) {
                  //  console.log(driver);
                  const e = await DistinationDuration(
